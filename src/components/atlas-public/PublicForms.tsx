...
export function LocalBrief({ contactMode = false }: { contactMode?: boolean }) {
  ...
  return <section id={contactMode ? "contact-summary" : "project-brief"} className={styles.formSection} aria-labelledby="brief-title">
    ...
    <form onSubmit={prepare} noValidate>
      ...
      <label htmlFor="brief-organisation">Organisation<input id="brief-organisation" name="organisation" autoComplete="organization" required aria-invalid={Boolean(errors.organisation)} aria-describedby={errors.organisation ? "brief-organisation-error" : undefined}/>{errors.organisation && <span id="brief-organisation-error" className={styles.fieldError}>{errors.organisation}</span>}</label>
      <label htmlFor="brief-name">Your name<input id="brief-name" name="name" autoComplete="name" required aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? "brief-name-error" : undefined}/>{errors.name && <span id="brief-name-error" className={styles.fieldError}>{errors.name}</span>}</label>
      <label htmlFor="brief-email">Email<input id="brief-email" name="email" type="email" autoComplete="email" required aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? "brief-email-error" : undefined}/>{errors.email && <span id="brief-email-error" className={styles.fieldError}>{errors.email}</span>}</label>
      ...
      <label className={styles.formWide} htmlFor="brief-challenge">What needs to work better?<textarea id="brief-challenge" name="challenge" rows={5} minLength={20} required aria-invalid={Boolean(errors.challenge)} aria-describedby={errors.challenge ? "brief-challenge-error" : undefined}/>{errors.challenge && <span id="brief-challenge-error" className={styles.fieldError}>{errors.challenge}</span>}</label>
