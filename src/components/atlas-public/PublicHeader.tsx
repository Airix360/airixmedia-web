...
const links = [
  ["Home", "/"], ["Work", "/work"], ["Services", "/services"], ["Publishing", "/publishing"],
...
export function PublicHeader() {
  ...
  return <header className={`${styles.header} ${scrolled ? styles.scrolled : ""}`}>
    <div className={styles.headerInner}>
      ...
      <nav className={styles.desktopNav} aria-label="Primary navigation">{links.map(([label, href]) => {
      ...
      <div className={styles.headerTools}>
        ...
        <button ref={trigger} className={`${styles.iconButton} ${styles.menuButton}`} onClick={() => setOpen(true)} aria-label="Open menu" aria-expanded={open}><Menu size={21}/></button>
      </div>
    </div>
    {open && <div ref={menu} className={styles.menuOverlay} role="dialog" aria-modal="true" aria-label="Site navigation">
      ...
      <nav>{links.map(([label, href], index) => {
        const active = href === "/" ? pathname === "/" : pathname.startsWith(href);
        return <Link key={href} onClick={() => setOpen(false)} href={href} aria-current={active ? "page" : undefined}><span>0{index + 1}</span>{label}</Link>;
      })}</nav>
      <div className={styles.menuUtilities}>
        <div className={styles.menuUtilityLinks}><Link href="/support">Support</Link><Link className={styles.menuEmergencyLink} href="/support/emergency">Emergency</Link><a href={contact.portal}>Client Portal</a></div>
        <ThemeControl className={styles.menuThemeControl} expanded />
      </div>
    </div>}
  </header>;
}
