...
export function PublicPage({ page, path }: { page: PublicPageRecord; path: string }) {
  ...
  return <div className={styles.site}>
    ...
    <main id="main-content">
      ...
      {showRepositories && <section className={styles.repositoryList} aria-labelledby="repositories-title"><span className={styles.kicker}>VERIFIED PUBLIC REPOSITORIES</span><h2 id="repositories-title">Inspect the work at source.</h2>{openSourceProjects.map((repo, index) => <a href={repo.url} key={repo.slug}><span>0{index + 1}</span><div><strong>{repo.name}</strong><p className={contrast.copy}>{repo.summary}</p></div><ArrowRight size={18}/></a>)}</section>}
