...
test("unknown detail routes return 404 and the lead API refuses simulated success", async ({ page, request }) => {
  ...
  const lead = await request.post("/api/leads", { multipart: { selectedForm: "general", fullName: "Review Owner", email: "owner@example.com", organisation: "Airix", enquiryType: "General question", message: "A complete provider-availability verification message.", privacyAcknowledgement: "true", sourceRoute: "/contact" } });
  expect(lead.status()).toBe(503);
  await expect.poll(async () => (await lead.json()).ok).toBe(false);
  await page.goto("/contact?form=project");
  await page.getByRole("dialog").getByRole("button", { name: "Submit enquiry" }).click();
...
test("capture functional review evidence", async ({ page, request }, info) => {
  ...
  const formDialog = page.getByRole("dialog", { name: "General Enquiry" });
  await formDialog.getByRole("button", { name: "Submit enquiry" }).click();
  await page.screenshot({ path: output("functional-form-errors.png"), animations: "disabled", scale: "css" });
  await formDialog.getByLabel("Organisation *").fill("Review Organisation");
  await formDialog.getByLabel("Full name *").fill("Review Owner");
  await formDialog.getByLabel("Email *").fill("owner@example.com");
  await formDialog.getByLabel("Enquiry type *").selectOption("General question");
  await formDialog.getByLabel("Message *").fill("We need a verified and maintainable public system handoff.");
  await formDialog.getByLabel(/I have read/).check();
  await formDialog.getByRole("button", { name: "Submit enquiry" }).click();
  await expect(formDialog.getByText("Not sent.")).toBeVisible();
  await expect(formDialog.getByRole("link", { name: "Continue by email" })).toHaveAttribute("href", /^mailto:/);
