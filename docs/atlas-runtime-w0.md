...
The Atlas typography source requests later comparison of multiple font pairings; W0 therefore defines semantic roles without claiming a final pairing. The organised Atlas asset foundation remains candidate material, so the runtime treats every registered item as opt-in internal material even though the canonical files now live under stable category paths.
