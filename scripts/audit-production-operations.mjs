...
const blocked = { attempted: false, acceptedByProvider: false, recipientConfirmed: false, reason: "blocked-before-send" };

write("configuration-readiness-report.json", { provider: "brevo", configurationTemplate: true, apiKeyInGit: false, apiCredentialUsable: false, applicationTopologyProvisioned: false, liveReady: false });
write("sender-verification-status.json", { logicalSender: "Airix Media notifications", providerApiAuthentication: "rejected", senderVerified: "unconfirmed", liveSendingAllowed: false });
write("mailbox-alias-status.json", { generalMailbox: "present", supportMailbox: "present", publishingAlias: "present", emergencyAlias: "present", operationsFallbackAlias: "present", notificationsAlias: "present", endToEndReceipt: "unconfirmed", independentlyObservableEmergencyFallback: "unconfirmed" });
write("dns-authentication-status.json", { mx: "present", spf: "present-provider-authorization-unconfirmed", dkim: "present-provider-status-unconfirmed", dmarc: "present-no-conflict-observed", dnsChanged: false });
...
for (const form of ["general", "project", "publishing", "booking", "support", "emergency"]) write(`synthetic-${form}-result.json`, { form, marker: "STAGING TEST — NO ACTION REQUIRED", ...blocked });
write("recipient-confirmation-matrix.json", { GeneralOperations: "not-requested", PublishingOperations: "not-requested", SupportOperations: "not-requested", EmergencyDutyOperator: "not-requested", OperationsFallback: "not-requested", namedIndividualsIncluded: false });
write("emergency-primary-result.json", { ...blocked, monitoringHours: "08:00–22:00 WAT daily", monitoringOwner: "Emergency Duty Operator" });
write("emergency-fallback-result.json", { ...blocked, aliasProvisioned: true, controlledFailureExecuted: false });
...
write("truthful-acknowledgement-report.json", { booking: "request-only", support: "delivery-not-ticket", emergency: "delivery-not-acceptance", fallbackSuccessDisclosed: true, monitoringHoursPresent: true, claim24x7: false });
...
write("focused-audit-report.json", { pass: Object.values(audits).every(Boolean), audits, legalCopyRewritten: false, liveDeliveryVerified: false });
